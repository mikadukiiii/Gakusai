...
public class InnerMetadataResolver {
    ...
    private static InnerMetadataResolverCallback mMetadataResolverCallback;
    ...
    public interface InnerMetadataResolverCallback {
        public void onInnerUrlMetadataReceived(String annUrl, String url, UrlMetadata urlMetadata);
    ...
    public static int findInnerUrlMetadata(Context context,
                                            InnerMetadataResolverCallback metadataResolverCallback,
                                            String annUrl,
                                            String url,
                                            int txPower,
                                            int rssi) {
        ...
        initialize(context);
        int bytes = requestInnerUrlMetadata(annUrl, url, txPower, rssi);
        return bytes;
    }
    ...
    private static int requestInnerUrlMetadata(String annUrl, String url, int txPower, int rssi) {
        if (!mIsInitialized) {
        ...
        JsonObjectRequest jsObjRequest = createUrlMetadataRequest(annUrl, jsonObj, false);
        // Queue the request
        mRequestQueue.add(jsObjRequest);

        return jsonObj.toString().getBytes().length;
    }
    ...
    private static JsonObjectRequest createUrlMetadataRequest(final String annUrl, JSONObject jsonObj, final boolean isDemoRequest) {
                        ...
                        try {
                            ...
                            if (foundMetaData.length() > 0) {
                                ...
                                for (int i = 0; i < foundMetaData.length(); i++) {
                                    ...
                                    downloadIcon(annUrl,urlMetadata);

                                    if (isDemoRequest) {
                                        //mMetadataResolverCallback.onDemoUrlMetadataReceived(id, urlMetadata);
                                    } else {
                                        mMetadataResolverCallback.onInnerUrlMetadataReceived(annUrl, id, urlMetadata);
                                    }
                                }
     ...
     */
    private static void downloadIcon(final String annUrl, final UrlMetadata urlMetadata) {
        ImageRequest imageRequest = new ImageRequest(urlMetadata.iconUrl, new Response.Listener<Bitmap>() {
            ...
            public void onResponse(Bitmap response) {
                ...
                mMetadataResolverCallback.onInnerUrlMetadataIconReceived(annUrl);
            }
        }, 0, 0, null, null);
